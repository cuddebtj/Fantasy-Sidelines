...
def schedule_clean(data):
    ...
    schedule = schedule.rename(
        columns={
            "game.venue.id": "venue.id", "game.venue.name": "venue.name", "game.venue.city": "venue.city",
            "game.venue.state": "venue.state", "game.venue.country": "venue.country", "game.venue.zip": "venue.zip",
            "game.venue.address": "veunue.address", "game.venue.capacity": "venue.capacity", 
            "game.venue.surface": "venue.surface", "game.venue.roof_type": "venue.roof_type",
            "game.venue.sr_id": "venue.sr_id", "game.venue.location.lat": "venue.location.lat",
            "game.venue.location.lng": "venue.location.lng", "game.team.id": "team.id", "game.team.name": "team.name",
            "game.team.alias": "team.alias", "game.team.game_number": "team.game_number", "game.team.sr_id": "team.sr_id",
            "game.opp.id": "opp.id", "game.opp.name": "opp.name", "game.opp.alias": "opp.alias", 
            "game.opp.game_number": "opp.game_number", "game.opp.sr_id": "opp.sr_id"})
...
def player_stats_clean(data):
    ...
    player_stats_df["player.name"] = player_stats_df["player.name"].str.replace("|".join([re.escape(s) for s in replace]), "", regex=True)
...
def practice_par(season_start, season_end):
        ...
        for week in weeks:
            ...
            try:
                ...
                for i in range(len(idx[:-1])):
                    ...
                    final_data = {}
                    final_data["team"] = [data[start-12] for i in range(start, end, 6)]
                    final_data["player.name"] = [data[i-5] for i in range(start, end, 6)]
                    final_data["injury"] = [data[i-4] for i in range(start, end, 6)]
                    final_data["date1"] = [data[start-9] for i in range(start, end, 6)]
                    final_data["date1.status"] = [data[i-3] for i in range(start, end, 6)]
                    final_data["date2"] = [data[start-8] for i in range(start, end, 6)]
                    final_data["date2.status"] = [data[i-2] for i in range(start, end, 6)]
                    final_data["date3"] = [data[start-7] for i in range(start, end, 6)]    
                    final_data["date3.status"] = [data[i-1] for i in range(start, end, 6)]
                ...
                final_df[["last.date", "game.status", "home.away", "opp"]] = final_df["game.status"].str.split(expand=True)
                final_df[["player.name", "player.pos"]] = final_df["player.name"].str.split(", ", 1, expand=True)
                ...
                final_df["game.status"] = final_df["game.status"].replace({None: "Full"})
                final_df["home.away"] = final_df["home.away"].replace({"vs": "home", "@": "away"})
                ...
                final_df["opp"] = final_df["opp"].replace({
                    ...
                    "Det": "DET", "Hou": "HOU", "Ind": "IND", "Jax": "JAC", "Mia": "MIA",
                    "Min": "MIN", "Oak": "OAK", "Phi": "PHI", "Pit": "PIT", "Sea": "SEA",
                    "Ten": "TEN", "Was": "WAS"})
    ...
    replace = [" III", " II", " IV", " V", " Jr.", " Sr.", " Sr", " Jr"]
    practice_par_df["player.name"] = practice_par_df["player.name"].str.replace("|".join([re.escape(s) for s in replace]), "", regex=True)
...
def weekly_stats(player_stats, player_snaps, player_practice):
    ...
    weekly_stats = player_stats.merge(player_snaps, 
                                      how="outer", 
                                      left_on=["player.name", "season.year", "week.sequence", "team.alias", "opp.alias"], 
    ...
    weekly_stats = weekly_stats.drop(["player_name", "season", "week", "team", "opp"], axis=1)
