...
def injury_scrape(season):
    ...
    for team in teams:
        ...
        final_df = pd.melt(
            df, id_vars=columns[0:2], value_vars=columns[2:], value_name="injury"
        )
        ...
        final_df[["date.opp", "week"]] = final_df["variable"].str.split(
            ": week_", 1, expand=True
        )
        final_df[["status", "injury"]] = final_df["injury"].str.split(
            ": ", 1, expand=True
        )
        final_df[["date", "opp"]] = final_df["date.opp"].str.split(
            "vs. ", 1, expand=True
        )
        final_df["date"] = (
            final_df["date"].astype(str) + "/" + final_df["season"].astype(str)
        )
    ...
    injury_df["player.name"] = injury_df["player.name"].str.replace(
        "|".join([re.escape(s) for s in replace]), "", regex=True
    )
